...
namespace Game.Engine.Skills.AdvancedWeaponTechniques
{
    ...
    class RollAndBackStabDecorator : SkillDecorator
    {
        public RollAndBackStabDecorator(Skill skill) : base("RollAndBackStab", 40, 5, skill)
        {
            ...
            PublicName = "COMBO - Roll and stab in the back [requires spear]: 0.2*Pr damage [incised] and then 0.3*Str + 0.3*Pr damage [cut] AND " + decoratedSkill.PublicName.Replace("COMBO: ", "");
        ...
        public override List<StatPackage> BattleMove(Player player)
        {
            ...
            response2.CustomText = "You use roll&stab in the back technique! (" + ((int)(0.2 * player.Precision)) + " incised damage, " + ((int)(0.3 * player.Strength) + (int)(0.3 * player.Precision)) + " cut damage)";
