...
{
    [Serializable]
    class WolfWoundedState : WolfState
    {
        ...
        {
            parentSession.SendText("This wolf is wounded. You can save him to gain new companion or kill it get new armor.");
            int choice = parentSession.GetListBoxChoice(new List<string>() { "Kill anmial (new hat)", "Hill for 20 gold (new companion)" });
            switch (choice)
            {
                case 0:
                    parentSession.SendText("\n You gain Wolf Hat");
                    parentSession.AddThisItem(Index.ProduceSpecificItem("item0124"));// wolf hat item
