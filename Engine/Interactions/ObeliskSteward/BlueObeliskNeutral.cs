...
namespace Game.Engine.Interactions.Built_In
{
    ...
    [Serializable]
    class BlueObeliskNeutral : IObeliskStrategy
    {
        public List<string> Execute(GameSession parentSession, bool complete, StewardEncounter myself, List<string> list)
        {
            ...
            switch (choice)
            {
                ...
                parentSession.SendColorText("5 point damage","blue");
                parentSession.SendColorText("You make a sacrifice, but it costs...", "blue");
        ...
        public List<string> Order(GameSession parentSession, StewardEncounter myself, List<string> list)
        {
            ...
            if (list.Count == 0)
            {
                if (myself.order[0] == "blue") list.Add("blue"); ;
            }
            else
            {
                for (int i = 0; i < list.Count; i++)
                    ...
                    if (list[i] == myself.order[i])
                    {
                        ...
                        else if ((i == list.Count - 1) & myself.order[i + 1] == "blue")
                        {
                            list.Add("blue");
                        }
                    }
